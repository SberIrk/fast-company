...
const dataUsers = api.users.fetchAll().reduce((calc,dataUser) =>
    Object.assign(calc,{[dataUser._id] : dataUser}),{});
    ...
    const _rowRender = (user) => {
        ...
        return (
            <tr key={user._id}>
                ...
                <td>{`${user.rate} / 5`}</td>
                <td><button type="button" className="btn btn-danger" onClick={() => _buttonDeleteUser(user._id)}>Удалить</button></td>
            </tr>
        );
    }
